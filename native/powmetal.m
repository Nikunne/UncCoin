...
#define METAL_NONCES_PER_THREAD 8U
...
static NSString *const POW_METAL_SOURCE =
    ...
    "    write_sha256_length(prepared.second_block, total_bit_length);\n"
    "}\n"
    "inline void sha256_digest_prepared(thread const SHA256Context &prefix_context,\n"
    "                                   thread const PreparedNonceBlocks &prepared,\n"
    "                                   thread uchar hash[SHA256_BINARY_LENGTH]) {\n"
    "    SHA256Context context;\n"
    "    for (uint i = 0; i < 8u; ++i) { context.state[i] = prefix_context.state[i]; }\n"
    "    context.bit_length = 0ul;\n"
    ...
    "    ulong candidate_index = (ulong)gid * (ulong)NONCES_PER_THREAD;\n"
    "    if (candidate_index >= params->nonce_count) { return; }\n"
    "    if (atomic_load_explicit(&result->found, memory_order_relaxed) != 0u) { return; }\n"
    "    ulong nonce = params->start_nonce + (candidate_index * params->nonce_step);\n"
    ...
    "    uint checked = 0u;\n"
    "    uint max_checks = NONCES_PER_THREAD;\n"
    "    ulong remaining = (ulong)params->nonce_count - candidate_index;\n"
    "    if (remaining < (ulong)max_checks) { max_checks = (uint)remaining; }\n"
    "    while (checked < max_checks) {\n"
    "        if (checked > 0u) {\n"
    "            uint previous_nonce_length = nonce_length;\n"
    "            int changed_index = increment_ascii_decimal(nonce_chars, nonce_length, params->nonce_step);\n"
    ...
    "            }\n"
    "        }\n"
    "        sha256_digest_prepared(prefix, prepared, digest);\n"
    "        if (has_leading_zero_bits(digest, params->difficulty_bits)) {\n"
        ...
        *out_cancelled = 0;
        ...
        while (true) {
            ...
            NSUInteger thread_count =
                (NSUInteger)((batch_size + (unsigned long long)METAL_NONCES_PER_THREAD - 1ULL) /
                             (unsigned long long)METAL_NONCES_PER_THREAD);
